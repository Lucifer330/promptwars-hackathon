...
### 1. Priority Queue & Binary Min-Heap (`src/tests/priorityQueue.test.ts`)
- **MinPriorityQueue Ordering**: Verifies min-heap extract-min operations maintain $O(\log N)$ priority ordering for Dijkstra pathfinding.
- **Duplicate Priority Handling**: Verifies queue stability under identical cost nodes.

### 2. Interactive Venue Navigation & Pathfinding (`src/tests/pathfinding.test.ts`, `src/tests/navigation.test.ts`)
...
### 3. Personalization & Recommendation Engine (`src/tests/recommendations.test.ts`, `src/tests/discovery.test.ts`)
...
- **Saved Session Exclusion**: Verifies that sessions already added to the user's agenda are automatically excluded from the recommendation stream to prevent duplicates ($O(1)$ `Set` check).
...
### 4. Crowd Density & SOS Incident Workflows (`src/tests/crowdAndSOS.test.tsx`)
...
### 5. Organizer Dashboard & Broadcast Actions (`src/tests/organizerActions.test.tsx`, `src/tests/eventContext.test.tsx`)
...
```text
 ...
 ✓ src/tests/priorityQueue.test.ts  (2 tests)
